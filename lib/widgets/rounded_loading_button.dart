...
class RoundedLoadingButtonState extends State<RoundedLoadingButton> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    var _check = Container(
        ...
        child: _bounceAnimation.value > 20
            ? Icon(
                widget.successIcon,
                size: getDeviceType(context, 15, 20, 25, 30),
    ...
    var _loader = SizedBox(
        ...
        child:
            CircularProgressIndicator(valueColor: AlwaysStoppedAnimation<Color>(widget.valueColor), strokeWidth: widget.loaderStrokeWidth));
    ...
    final _btn = ButtonTheme(
        ...
        child: ElevatedButton(
          style: ElevatedButton.styleFrom(
            ...
            side: BorderSide(width: getDeviceType(context, 1, 1, 2, 2), color: Colors.white),
  ...
  void initState() {
    ...
    _bounceAnimation =
        Tween<double>(begin: 0, end: widget.height).animate(CurvedAnimation(parent: _checkButtonControler, curve: widget.completionCurve));
    _bounceAnimation.addListener(() {
      setState(() {});
    });

    _squeezeAnimation =
        Tween<double>(begin: widget.width, end: widget.height).animate(CurvedAnimation(parent: _buttonController, curve: widget.curve));
